...
$categorys = array( "PD" => array(), //Falls nur ein Bild übermittelt wird, kann diese Kat verwendet werden.
	"CC-by-sa" => array(), "CC-by" => array(), "CC-sa" => array(), "GFDL" => array(), "FWL" => array(), "MPL" => array(), "FAL" => array(), "GPL" => array(), "GPLV2ONLY" => array(), "LGPL" => array(), "CeCILL" => array(), "Attribution" => array(), "CopyrightByWikimedia" => array() );
...
if ( $isaccord == true ) {
	...
	foreach ( $images as $imagename => $licarray ) {
		...
		if ( $tempcache["query"]["pages"][$arkey[0]] == "-1" ) {
			die( "<div class='notexist'>ERROR - $imagename not found!</div>" );
		}
...
//Templates
	$licensesar = array( "Attribution" => "Attribution", "CC-by-sa" => "Cc-by-sa-3.0", "CC-by" => "Cc-by-3.0", "CC-sa" => "cc-sa-1.0", "GFDL" => "GFDL", "PD" => "PD-self", "FWL" => "FWL", "MPL" => "MPL", "FAL" => "FAL", "GPL" => "GPL", "GPLV2ONLY" => "GPLv2 only", "LGPL" => "LGPL", "CeCILL" => "CeCILL", "CopyrightByWikimedia" => "CopyrightByWikimedia" );
...
if ( $isaccord == false ) {
	...
	foreach ( $images as $imagename => $licarray ) {
		...
		$starset1 = "";
		if ( $imagelizok[$imagename] == false ) {
			$starset1 = "<sup class='marker'>2</sup>";
		}
		echo "<li>$imagename $starset1</li><ul>";
		foreach ( $licarray as $liz ) {
			$starset = "";
			if ( $foroutstat[$liz] == false ) {
				$starset = "<sup class='marker'>1</sup>";
			}
